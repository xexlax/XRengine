...
namespace Ubpa::detail {
    ...
    template<typename C, std::size_t N, fcstr<C, N> str> struct TStr {
...
namespace Ubpa::USRefl {
    ...
    template<class Name, class T>struct NamedValue : NamedValueBase<Name> {
        T value; static constexpr bool has_value = true;
    ...
    template<typename...Es> struct ElemList {
        ...
        template<class S> constexpr const auto& Find(S = {}) const {
            constexpr std::size_t idx = []() { constexpr decltype(S::View()) names[]{ Es::name... };
